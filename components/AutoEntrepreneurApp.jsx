...
function genererCalendrier(profil) {
  ...
  if (regime === 'mensuel') {
    for (let i = -2; i <= 4; i++) {
      ...
      events.push({
        id: periode, periode, type: 'mensuel',
    ...
    for (let i = -1; i <= 2; i++) {
      ...
      events.push({
        id: periode, periode, type: 'trimestriel',
  ...
  // CFE décembre
  events.push({
    id: `CFE-${year}`, periode: `${year}-12`, type: 'cfe',
    label: `CFE — Cotisation Foncière des Entreprises ${year}`,
    date_limite: `15/12/${year}`, past: false, current: false, special: true,
  })

  // IR
  events.push({
    id: `IR-${year}`, periode: `${year}-05`, type: 'ir',
    label: `Déclaration Impôt sur le Revenu ${year}`,
    date_limite: `31/05/${year+1}`, past: false, current: false, special: true,
  })
...
export default function AutoEntrepreneurApp({ user, onLogout }) {
  ...
  const [oForm, setOForm] = useState({
    ...
    acre: false, acre_fin: '', objectif_ca: ''
  })
  ...
  const loadAll = async () => {
    ...
    if (p) { setProfil(p); setOForm(f => ({...f,...p})) }
  ...
  const saveRevenu = async () => {
    ...
    setRevenus(prev => {
      const filtered = prev.filter(r => r.mois !== revMois)
      return [data, ...filtered].sort((a,b) => b.mois.localeCompare(a.mois))
    })
    setRevMois(''); setRevMontant('')
    setSavingRev(false)
  }
  ...
  const marquerDeclaration = async (periode, type, statut) => {
    const { year, month } = getNow()
    const data = {
      user_id: user.id, periode, type_periode: type, statut,
  ...
  const calculer = () => {
    ...
    const impots_estimes = ca * 0.14 // taux moyen IR
    ...
    <>
        ...
        <div className="overlay show" onClick={e => { if(profil && e.target.className.includes('overlay')) setShowOnboarding(false) }}>
          <div className="modal" style={{maxWidth:560}}>
            ...
            <div className="form-grid">
              ...
              <div className="field full"><label>Activité (décris ce que tu fais) *</label><input value={oForm.activite} onChange={e=>setOForm(p=>({...p,activite:e.target.value}))} placeholder="Développeuse web freelance, graphiste, plombier…" /></div>
              ...
              <div className="field">
                <label>Régime de déclaration URSSAF</label>
              ...
              {oForm.acre && (
                <div className="field full"><label>Date de fin ACRE</label><input type="date" value={oForm.acre_fin} onChange={e=>setOForm(p=>({...p,acre_fin:e.target.value}))} /></div>
              )}
            </div>
          ...
          <div className="card" style={{marginBottom:'1.5rem'}}>
            ...
            <div className="seuil-item">
              <div className="seuil-top">
                ...
                <span className="seuil-val">{caAnnuel.toLocaleString('fr-FR')} € / {seuil_tva.toLocaleString('fr-FR')} €</span>
              </div>
              <div className="progress-bar">
                <div className="progress-fill" style={{width:pctTVA+'%', background: pctTVA > 85 ? '#C0392B' : pctTVA > 60 ? '#B5792A' : '#2D7A4F'}}/>
              </div>
            ...
            <div className="seuil-item" style={{marginTop:'1rem'}}>
              <div className="seuil-top">
                ...
                <span className="seuil-val">{caAnnuel.toLocaleString('fr-FR')} € / {plafond.toLocaleString('fr-FR')} €</span>
              </div>
              <div className="progress-bar">
                <div className="progress-fill" style={{width:pctPlafond+'%', background: pctPlafond > 85 ? '#C0392B' : pctPlafond > 60 ? '#B5792A' : '#2D7A4F'}}/>
              </div>
        ...
        <div className="main">
          ...
          <div className="info-box" style={{marginTop:'1.5rem'}}>
            ...
            <div className="info-text">
              1. Va sur <strong>autoentrepreneur.urssaf.fr</strong><br/>
          ...
          <div className="card" style={{marginBottom:'1.5rem'}}>
            ...
            <div style={{display:'flex',gap:12,flexWrap:'wrap',alignItems:'flex-end'}}>
              ...
              <div>
                ...
                <input className="mini-input" type="number" value={revMontant} onChange={e=>setRevMontant(e.target.value)} placeholder="3 500" style={{width:160}} />
              </div>
              ...
              <div className="card" style={{marginBottom:'1.5rem'}}>
                ...
                <div style={{display:'flex',gap:12,alignItems:'flex-end',flexWrap:'wrap'}}>
                  <div>
                    ...
                    <input className="mini-input" type="number" value={calcCA} onChange={e=>setCalcCA(e.target.value)}
      ...
      {view === 'assistant' && (
        <div className="main">
          ...
          <div className="card" style={{marginBottom:'1.5rem'}}>
            <div className="questions-rapides">
            ...
            <div className="input-wrap" style={{marginTop:'1rem'}}>
              <textarea value={question} onChange={e=>setQuestion(e.target.value)}
                onKeyDown={e => { if(e.key==='Enter'&&(e.metaKey||e.ctrlKey)) poserQuestion() }}
          ...
          {/* Réponse */}
          {(reponse || asking) && (
            <div className="card reponse-card">
              ...
              {asking ? (
                <div style={{display:'flex',alignItems:'center',gap:10,padding:'1rem 0',color:'#A89878'}}>
                  <div className="ring"/>Je réfléchis à ta question…
                </div>
              ) : (
